package com.datagraphice.fcriscuo.alsdb.graphdb.supplier;
...
public class GraphDatabaseServiceSupplier implements Supplier<GraphDatabaseService> {
  ...
  private Consumer<Path> configureTestGraphConsumer = (path) -> {
    try {
      ...
      com.datagraphice.fcriscuo.alsdb.graphdb.util.Utils.deleteDirectoryAndChildren(path);
    } catch (IOException e) {
      e.printStackTrace();
    }
  ...
  public GraphDatabaseServiceSupplier(RunMode runMode) {
    switch (runMode) {
      case TEST:
        com.datagraphice.fcriscuo.alsdb.graphdb.util.FrameworkPropertyService.INSTANCE.getOptionalPathProperty("testing.db.path")
            .ifPresent(configureTestGraphConsumer);
        AsyncLoggingService.logInfo("new test ALS Neo4j database will be created.");
        break;
      case PROD:
        com.datagraphice.fcriscuo.alsdb.graphdb.util.FrameworkPropertyService.INSTANCE.getOptionalPathProperty("neo4j.db.path")
  ...
  public static void main(String[] args) {
    com.datagraphice.fcriscuo.alsdb.graphdb.util.FrameworkPropertyService.INSTANCE.getOptionalPathProperty("readonly.db.path")
        .ifPresent(path -> {
