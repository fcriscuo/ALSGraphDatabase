...
import kotlin.Pair;
import org.biodatagraphdb.alsdb.lib.DynamicRelationshipTypes;
...
public abstract class GraphDataConsumer implements Consumer<Path> {
    ...
    protected GraphDataConsumer(@Nonnull org.biodatagraphdb.alsdb.service.graphdb.RunMode runMode) {
        this.lib = new FunctionLib(runMode);
        this.runMode = runMode;
    }

    public Supplier<org.biodatagraphdb.alsdb.service.graphdb.RunMode> consumerRunModeSupplier = () -> this.runMode;
   ...
   */
    ...
    protected Function<Pair<String, String>, Node> resolveEventTimepointNodeFunction = (pair) -> {
        Node node = lib.resolveGraphNodeFunction.apply(new Tuple3<>(alsStudyTimepointLabel, "TimepointId", pair.getFirst()));
        lib.nodePropertyValueConsumer.accept(node, new Tuple2<>("TimepointName", pair.getSecond()));
        return node;
    };
    /*
    resolveSubjectEventNodeFunction.apply(property.subjectEventTuple());
     */
    protected Function<Pair<String, String>, Node> resolveSubjectEventNodeFunction = (pair) -> {
        Node node = lib.resolveGraphNodeFunction.apply(new Tuple3<>(alsStudyTimepointEventLabel,
                "EventCategory", pair.getFirst()));
        lib.nodePropertyValueConsumer.accept(node, new Tuple2<>("FormName", pair.getSecond()));
        return node;
    };
    ...
    protected Function<Pair<String, String>, Node> resolveSubjectNodeFunction =
            (subjectPair) -> {
                Node node = lib.resolveGraphNodeFunction.apply(new Tuple3<>(subjectLabel,
                        "SubjectGuid", subjectPair.getSecond()));
                lib.nodePropertyValueConsumer.accept(node, new Tuple2<>("SubjectId", subjectPair.getFirst()));
                return node;
            };
    ...
    protected Function<RnaTpmGene, Node> resolveRnaTpmGeneNode = (rnaTpmGene) -> {
        Node node = lib.resolveGraphNodeFunction.apply(new Tuple3<>(rnaTpmLabel,
                "RnaTpmId", rnaTpmGene.getId()));
        if (node != null) {
            // persist tpm value as a String
            lib.nodePropertyValueConsumer
                    .accept(node, new Tuple2<>("TPM", String.valueOf(rnaTpmGene.getTpm())));
        }
        return node;
    };
    ...
    protected void createEnsemblTranscriptNodes(@Nonnull org.biodatagraphdb.alsdb.model.UniProtValue upv) {
        String uniprotId = upv.getUniprotId();
        upv.getEnsemblTranscriptList().forEach(transcriptId -> {
    ...
    protected Function<SampleVariantSummary, Node> resolveSampleVariantNode = (svc) -> {
        ...
        Node svNode = lib.resolveGraphNodeFunction
                .apply(new Tuple3<>(sampleVariantLabel, "SampleVariantId", svc.getId()));
        // persist the list of variants
        lib.nodePropertyValueStringListConsumer.accept(svNode, new Tuple2<String, List<String>>("Variants", svc.getVariantList()));
        return svNode;
    };
