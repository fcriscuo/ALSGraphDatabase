  ...
  */
  private Function<RnaTpmGene,Node> resolveRnaTpmGeneNode=  (rnaTpmGene) -> {
    ...
    if (!rnaTpmGeneMap.containsKey(id)){
      ...
      nodePropertyValueConsumer.accept(node,new Tuple2<>("TPM",String.valueOf(rnaTpmGene.tpm())));
  ...
  private Consumer<RnaTpmGene> rnaTpmGeneConsumer = (tpm) -> {
      ...
      Node hugoGeneNode = resolveHugoGeneNodeFunction
    ...
    createBiDirectionalRelationship(proteinNode,rnaNode, new Tuple2<>(tpm.uniProtMapping().get().uniProtId(), tpm.id()),
        proteinTPMRelMap, RelTypes.EXPRESSION_LEVEL, RelTypes.EXPRESSED_PROTEIN
        );
    createBiDirectionalRelationship(proteinNode, hugoGeneNode, new Tuple2<>(tpm.uniProtMapping().get().uniProtId(),
        tpm.uniProtMapping().get().geneSymbol()), proteinXrefRelMap, RelTypes.MAPS_TO, RelTypes.MAPS_TO);
    createBiDirectionalRelationship(proteinNode, ensemblGeneNode, new Tuple2<>(tpm.uniProtMapping().get().uniProtId(),
        tpm.uniProtMapping().get().ensemblGeneId()), proteinXrefRelMap, RelTypes.MAPS_TO, RelTypes.MAPS_TO);
    createBiDirectionalRelationship(proteinNode, ensemblTranscriptNode, new Tuple2<>(tpm.uniProtMapping().get().uniProtId(),
        tpm.uniProtMapping().get().ensemblTranscriptId()), proteinXrefRelMap, RelTypes.MAPS_TO, RelTypes.MAPS_TO);
  };
